...
function auth(req,res,next) {
  ...
  req.token = token; req.userId = userId; req.user = user; req.db = db;
  next();
}
function save(req) { req.db.users[req.userId] = req.user; write(req.db); }
...
app.post('/api/signup', (req,res) => {
  ...
  const db = read(); const e = norm(email); const p = norm(phone);
  if (Object.values(db.users).some(u => norm(u.email)===e || norm(u.phone)===p)) return res.status(409).json({ message:'An account with that email or phone already exists.' });
...
app.post('/api/login', (req,res) => {
  ...
  const db = read(); const value = norm(identity);
  const user = Object.values(db.users).find(u => norm(u.email)===value || norm(u.phone)===value);
...
app.get('/api/me/data', auth, (req,res) => {
  ...
  if (settleMaturedInvestments(req.user.data)) save(req);
...
app.post('/api/test/fund', auth, (req,res) => {
  ...
  save(req); res.json({ ok:true, message:`Demo funding verified. £${amount.toFixed(2)} added.` });
});

app.get('/api/test/tasks', auth, (req,res) => res.json({ tasks:TASKS }));
app.post('/api/test/tasks/:id/submit', auth, (req,res) => {
  ...
  save(req); res.json({ ok:true, message:`Task completed. £${task.reward.toFixed(2)} demo reward added.` });
});

app.post('/api/test/withdraw', auth, (req,res) => {
  ...
  save(req); res.json({ ok:true, message:'Your withdrawal has been processed.' });
});

app.post('/api/test/invest', auth, (req,res) => {
